...
def CATEGORIES():
   ...
   #CreateDir('[COLOR white][B]TOOLS[/B][/COLOR]','ur','tools',ADDON_ICON,ADDON_FANART,'', isFolder=True)
    ...
    CreateDir('[COLOR white][B]Sauvegarde/Restauration[/B][/COLOR]','ur','backup_restore',ADDON_ICON,ADDON_FANART,'')
    CreateDir('[COLOR white][B]Advanced settings (Mémoire tampon)[/B][/COLOR]','ur', 'adv_settings', ADDON_ICON,ADDON_FANART,'')
    CreateDir('[COLOR white][B]Voir/télécharger fichier journal[/B][/COLOR]','ur', 'log_tools', ADDON_ICON,ADDON_FANART,'')
    CreateDir('[COLOR white][B]Speedtest[/B][/COLOR]','ur', 'speedtest', ADDON_ICON,ADDON_FANART,'')
    CreateDir('[COLOR white][B]Réglages[/B][/COLOR]','ur','settings',ADDON_ICON,ADDON_FANART,'')
...
def MAINTENANCE():
    ...
    CreateDir('Prochain nettoyage: %s' % nextAutoCleanup,'xxx','xxx',None,ADDON_FANART,'',isFolder=False,iconImage='DefaultIconInfo.png')
    CreateDir('Nettoyer le cache','url','clear_cache',ADDON_ICON,ADDON_FANART,'')
    CreateDir('Nettoyer les packages','url','clear_packages',ADDON_ICON,ADDON_FANART,'')
    CreateDir('Nettoyer les vignettes','url','clear_thumbs',ADDON_ICON,ADDON_FANART,'')
...
def FRESHSTART(mode='verbose'):
    if mode != 'silent': select = xbmcgui.Dialog().yesno("EZ Maintenance Mod", 'Are you absolutely certain you want to wipe this install?' + '\n' + 'All addons EXCLUDING THIS WIZARD will be completely wiped!', yeslabel='Yes',nolabel='No')
    else: select = 1
    if select == 0: return
    elif select == 1:
...
if action   == None: CATEGORIES()
    ...
    xbmc.executebuiltin('Runscript("special://home/addons/script.ezmaintenancemod/resources/lib/modules/speedtest.py")')
