...
if int(total_sizetext) > filesize:
    choice2 = xbmcgui.Dialog().yesno("[COLOR=red]Nettoyage automatique[/COLOR]", 'Le dossier packages est de [COLOR red]' + str(total_sizetext) +' MB [/COLOR] - [COLOR red]' + str(count) + '[/COLOR] fichiers zip' + '\n' + 'Le dossier peut être nettoyé sans problème pour économiser de l\espace...' + '\n' + 'Voulez-vous le nettoyer maintenant?', yeslabel='Oui',nolabel='Non')
    if choice2 == 1:
...
if int(total_sizetext2) > filesize_thumb:
    choice2 = xbmcgui.Dialog().yesno("[COLOR=red]Nettoyage automatique[/COLOR]", 'Le dossier de vignettes est de [COLOR red]' + str(total_sizetext2) + ' MB   [/COLOR]' + '\n' + 'Le dossier peut être nettoyé sans problème pour économiser de l\espace...' + '\n' + 'Voulez-vous le nettoyer maintenant?', yeslabel='Oui',nolabel='Non')
    if choice2 == 1:
