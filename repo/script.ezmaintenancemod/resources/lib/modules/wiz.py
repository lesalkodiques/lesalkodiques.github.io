    ...
    if os.path.exists(BACKUPDATA):
        if not backupdir == '':
            ...
            if name != "-":
                ...
                exclude_dirs = ['']
                canceled = CreateZip(BACKUPDATA, backup_zip, 'Création de la sauvegarde', 'Sauvegarde des fichiers', exclude_dirs, exclude_database)
                if canceled:
                    os.unlink(backup_zip)
                    dialog.ok(AddonTitle,'Sauvegarde annulée')
                else:
                    dialog.ok(AddonTitle,'Sauvegarde terminée')
        else:
           dialog.ok(AddonTitle,'Aucun emplacement de sauvegarde trouvé : veuillez configurer votre emplacement de sauvegarde')
...
def restoreFolder():
    ...
    if zipFolder == '' or zipFolder == None:
        control.infoDialog('Veuillez d\abord configurer un emplacement de fichiers Zip')
...
def restore(zipFile):
    yesDialog = dialog.yesno(AddonTitle, 'Cela écrasera tous vos paramètres actuels ... Êtes-vous certain?', yeslabel='Oui', nolabel='Non')
    if yesDialog:
        try:
            dp = xbmcgui.DialogProgress()
            dp.create("Restauration des fichiers","En cours..." + '\n' + "Veuillez patienter")
            dp.update(0, "" + '\n' + "Extraction du zip Veuillez patienter")
            canceled = ExtractZip(zipFile, control.HOME, dp)
            if canceled:
                dialog.ok(AddonTitle,'Restauration annulée')
            else:
                dialog.ok(AddonTitle,'Restauration complètee')
            xbmc.executebuiltin('Fermeture')
        except:pass
    ...
    for dirpath, dirnames, filenames in os.walk(folder):
        ...
        try:
            ...
            for file in filenames:
                ...
                if PY2:
                    dp.update(int(progress),"Sauvegarde", 'FICHIERS: ' + str(count) + '/' + str(N_ITEM)  + '   [COLOR lime]' + str(file) + '[/COLOR]', 'Veuillez-patienter')
                else:
                    dp.update(int(progress),"Sauvegarde" + '\n' + 'FICHIERS: ' + str(count) + '/' + str(N_ITEM)  + '   [COLOR lime]' + str(file) + '[/COLOR]' + '\n' + 'Veuillez-patienter')
...
def ExtractWithProgress(_in, _out, dp):
    ...
    try:
        for item in zin.infolist():
            ...
            if PY2:
                dp.update(int(update),'Extraction... Erreurs:  ' + str(errors) , label, '')
            else:
                dp.update(int(update),'Extraction... Erreurs:  ' + str(errors) + '\n' + label)
            try: zin.extract(item, _out)
            except Exception as e:
                print ("ERREURS D\EXTRACTION", e)
                pass
...
def buildInstaller(url):
    destination = dialog.browse(type=0, heading='Sélectionnez Répertoire de téléchargement', shares='fichiers',useThumbs=True, treatAsFolder=True, enableMultiple=False)
    if destination:
